...
> 最近 openclaw、hermes-agent 都挺火热的，在 iOS 平台也只有更轻量的 Minis 可以代替他们工作。这次也是借用 Minis 来帮忙在阿里云服务器上部署了基于 Astro 框架和 Firefly 主题的博客。
---
## 一、缘起
...
5. 测试自动化
为了验证成果，我在本地对博客做了一个小小的修改，然后将改动 `git push` 到了 `master` 分支。
...
## 七、最后的总结
从一时兴起租了服务器，到借助 Minis 和1Panel面板完成部署，再到最后配置好GitHub Actions实现自动化，整个过程比我想象的要顺利。
...
我最初是直接在 `master` 分支上修改所有东西，然后设置GitHub Actions从 `master` 分支部署。这样做虽然直接，但有一个隐患：如果我 fork 的源项目（Firefly主题）发布了更新，我想同步这些更新时，直接在 `master` 分支上操作会非常混乱，很容易产生代码冲突。

更规范的做法应该是这样：

1.	`master` 分支保持纯净： 这个分支只用来从上游的源项目拉取更新。我不应该在上面做任何个性化修改。
2.	创建开发分支： 我应该从 `master` 分支创建一个新的分支，比如叫做 `blog-dev`。
3.	在开发分支上工作： 我所有的新文章、个性化修改和配置，都应该在这个 `blog-dev` 分支上进行。
4.	修改部署流程： 将GitHub Actions的触发条件从 `master` 分支改成 `blog-dev` 分支。

这样一来，当源项目有更新时，我只需要切换到 `master` 分支，拉取更新，然后再将这些更新合并到我的 `blog-dev` 分支里。整个流程会清晰很多，也不容易出错。

算是一个小小的教训，也是折腾之后的一点收获吧。